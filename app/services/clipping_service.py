...
class ClippingService:
        ...
        *,
        ...
        for position, item in enumerate(items, start=1):
            payload.append({
                ...
                "position": position,
                "source": item.get("source") or "Fonte não identificada",
                "title": item.get("title") or "Sem título",
                "url": item.get("url") or "",
                "matched_term": item.get("term") or item.get("matched_term"),
    ...
    async def get(self, clipping_id: str) -> dict | None:
        rows = await self.database.request(
            "GET",
            "clippings",
            params={"select": "*", "id": f"eq.{clipping_id}", "limit": "1"},
        )
